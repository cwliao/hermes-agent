...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        # Request overrides are runtime-specific. Keep caller-supplied keys
        # while the provider-specific re-resolution below removes only fields
        # contributed by the old custom provider. In particular, do not reset
        # the whole mapping here: doing so loses caller-owned nested
        # ``extra_body`` fields such as reasoning effort.
        ...
        try:
            ...
            # A primary runtime may have a provider-specific vLLM thinking
            # toggle without a custom-provider entry (for example a direct
            # local endpoint). It must not make a genuine thinking fallback
            # look non-thinking, but unrelated caller keys must survive.
            if not _old_provider_eb and isinstance(_existing_eb, dict):
                _chat_template = _existing_eb.get("chat_template_kwargs")
                if isinstance(_chat_template, dict) and "enable_thinking" in _chat_template:
                    _chat_template = dict(_chat_template)
                    _chat_template.pop("enable_thinking", None)
                    _scrubbed = dict(_existing_eb)
                    if _chat_template:
                        _scrubbed["chat_template_kwargs"] = _chat_template
                    else:
                        _scrubbed.pop("chat_template_kwargs", None)
                    if _scrubbed:
                        _overrides["extra_body"] = _scrubbed
                    else:
                        _overrides.pop("extra_body", None)
                    _existing_eb = _scrubbed
