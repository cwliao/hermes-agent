...
MEMORY_GUIDANCE = (
    ...
    "Do not use memory as a document or bulk-content store. If the user asks you to "
    "create, merge, or repair a document, use write_file or patch, then read it back "
    "and verify its size/content before claiming success. Do not replace a failed "
    "file-tool call with a shell heredoc targeting ~/.hermes; if the correct file "
    "tool is unavailable, report that blocker instead of writing a partial artifact.\n"
...
KANBAN_GUIDANCE = (
    ...
    "routing, not implementation. For parallel comparisons, use "
    "`kanban_swarm`, not `kanban_create`.\n"
    "A new user request is a new logical Kanban operation. Never reuse, block, "
    "or report an old `done`/`archived` card as the result of the current turn "
    "unless the user explicitly names that card and asks to inspect or resume "
    "it. For a new request, create a fresh task or call `kanban_swarm`; an old "
    "similar title is not proof that the current work is complete.\n"
    "Never create a substitute or workaround task for in-flight or stuck swarm "
    ...
    "Never copy a `worker_max_runtime_seconds` value from a compacted summary's "
    "`Constraints` into a `kanban_swarm` call — it may be stale vs. the schema's "
    "live default. Omit it unless the user's CURRENT turn restates the value.\n"
    "\n"
    ...
    "- Do not create substitute or workaround tasks for in-flight or stuck swarm "
    "work. If a previously-launched swarm referenced in this conversation is "
    "running, blocked, or timing out, use `kanban_show` on its synthesizer or "
    "root task, wait, or report its true status truthfully — never spin up a "
    "similar-goal task and report its result as if it were that swarm's completion.\n"
