...
- **Do not assign kanban tasks to `orchestrator-claude`; use `worker-claude` instead** (confirmed
  2026-09-24, see `docs/HANDOVER-KANBAN-SWARM-CLAUDE-LANE-TOOL-PROTOCOL-2026-09-24.md`).
  `orchestrator-claude` routes through clawo's openai-compat HTTP stub (`claude -p --tools ""`), but
  `--tools ""` only disables Claude Code's built-in tools, not its profile's configured MCP servers
  (`sqlite-tasks`, `klib`, `notion`, `agentmemory`, ...) — needs `--strict-mcp-config` too, which the
  stub doesn't pass. Claude sees genuine native tools present, tries `kanban_complete` as a native
  call instead of the prompt-only `<tool_calls>` emulation, and gets a real "No such tool available"
  rejection every time. `worker-claude` is an `hermes profile create --clone-from orchestrator-claude`
  clone with the `mcp_servers:` block removed from its `config.yaml` — same model/skills/`.env`,
  no leaked MCP tools, verified to complete kanban tasks on the first attempt.
  `orchestrator-claude` itself is untouched and still fine for its non-kanban orchestrator role.
  For a one-off Claude review outside a kanban swarm, `agentpool/dispatch.js` with
  `preferred_engine: "claude"` (drives `clawo session-start`) also still works.
