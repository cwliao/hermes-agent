...
- [ ] 針對上面 4 個方向（或其組合），走真實 instamem 共識，決定
      實際要採用的策略
- [ ] 共識定案後，另開實作票，不在本票範圍內直接動手
- [ ] `2026-09-18-upstream-rebase-1356-commits-002.md` 這次的衝突解
      完、部署完之後，回頭在本票補記錄：這次解決的具體衝突內容能不能
      印證/修正上面「熱點檔案」的判斷（例如這次是否還是撞在
      `approval_gateway_wait.py`，還是換了別的檔案）
