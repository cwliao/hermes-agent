...
class GatewayInboundMixin:
    ...
    def _last30days_mode_from_choice(choice: str) -> Optional[str]:
        return {
            "1": "quick", "１": "quick", "quick": "quick", "快速": "quick", "快速摘要": "quick",
            "2": "deep", "２": "deep", "deep": "deep", "深入": "deep", "深入整理": "deep",
            "3": "sources", "３": "sources", "source": "sources", "sources": "sources",
            "指定": "sources", "指定來源": "sources",
    ...
    def _parse_last30days_source_codes(text: str) -> list[str]:
        mapping = {
            ...
            "th": "threads", "thread": "threads", "threads": "threads",
        ...
        tokens = [tok for tok in re.split(r"[\s,，、/]+", (text or "").strip().lower()) if tok]
        sources: list[str] = []
        for token in tokens:
    ...
    def _last30days_engine_sources(
        ...
        return ["reddit", "youtube", "instagram", "threads", "hackernews", "polymarket", "github", "grounding"]
    ...
    def _write_last30days_plan_file(topic: str, sources: list[str]) -> str:
        allowed_sources = {"grounding", "youtube", "reddit", "hackernews", "github", "polymarket", "x", "instagram", "threads"}
    ...
    def _normalize_last30days_engine_output(text: str) -> str:
        ...
        for raw_line in cleaned.splitlines():
            ...
            lowered = line.lower()
            if "tiktok and instagram" in lowered or lowered.startswith("bonus:"):
                continue
            kept.append(line)
        return "\n".join(kept).strip()
    ...
    def _clean_last30days_engine_output(text: str, *, max_chars: int = 3600) -> str:
        cleaned = GatewayInboundMixin._normalize_last30days_engine_output(text)
        if not cleaned:
    ...
    def _last30days_local_summary_model(self) -> str:
        try:
            ...
            model = ""
        return str(os.environ.get("LAST30DAYS_SUMMARY_MODEL") or model or "ornith:9b")
    ...
    async def _call_local_last30days_summary_llm(self, prompt: str) -> str:
        ...
        payload = {
            ...
            "messages": [
                {"role": "system", "content": "你是嚴格的研究摘要器。只能根據使用者提供的資料摘要，不得加入未出現在資料中的內容。請使用繁體中文。"},
    ...
    async def _run_last30days_telegram_report(
        self, *, topic: str, mode: str, sources: Optional[list[str]] = None
    ) -> str:
    ...
    async def _prompt_for_last30days_mode(
        self, *, source: SessionSource, session_key: str, topic: str
    ) -> None:
        ...
        await self._deliver_platform_notice(source, self._format_last30days_mode_prompt(topic))

    async def _handle_pending_last30days_choice(self, event: MessageEvent) -> Optional[str]:
        ...
        if step == "choose_sources":
            ...
            if not sources:
                await self._deliver_platform_notice(source, self._format_last30days_source_prompt(topic))
                return ""
            self._pending_last30days_choices().pop(key, None)
            await self._deliver_platform_notice(source, f"好，開始直接查詢「{topic}」。")
            report = await self._run_last30days_telegram_report(topic=topic, mode="quick", sources=sources)
        ...
        await self._deliver_platform_notice(source, f"好，開始直接查詢「{topic}」({label})。")
        report = await self._run_last30days_telegram_report(topic=topic, mode=mode)
        await self._deliver_platform_notice(source, report)
        return ""
    ...
    async def _extract_business_card_correction(
        ...
        prompt = (
            ...
            "使用者沒有要求修改的欄位。\n"
            "所有中文欄位一律輸出繁體中文（台灣用語與正體字）；若使用者輸入"
            "混入簡體字或簡繁混雜字形，轉換成對應繁體正字再填入欄位——這是"
            "字形校正，不是臆測新內容。\n"
            "只能輸出 JSON object，不要 markdown。\n\n"
            "欄位（鍵名必須完全相同）："
    ...
    async def _extract_business_card_fields(self, ocr_text: str) -> Dict[str, str]:
        ...
        prompt = (
            ...
            "所有中文欄位一律輸出繁體中文（台灣用語與正體字），這是台灣的名片"
            "資料庫。若 OCR 原文因掃描品質混入簡體字或簡繁混雜的字形（例如"
            "「吴」「国」「台湾」），必須轉換成對應的繁體正字（「吳」「國」"
            "「臺灣」／「台灣」）再填入欄位；這是字形校正，不是內容臆測，跟"
            "上面『不要修正數字』的限制無關，僅適用於中文字元本身。\n"
    ...
    def _save_business_card_local_backup(
        ...
        try:
            ...
            _home_fn = getattr(_gateway_run, "get_hermes_home", get_hermes_home)
            _gdrive_dir = getattr(_gateway_run, "_NAMECARD_GDRIVE_DIR", _NAMECARD_GDRIVE_DIR)
        except Exception:
            _home_fn = get_hermes_home
            _gdrive_dir = _NAMECARD_GDRIVE_DIR
        namecard_root = Path(_home_fn()) / "namecards"
    ...
    async def _save_business_card_to_notion(
        ...
        *,
        ...
        return {"url": page_url, "page_id": page_id, "duplicates": duplicates}
    ...
    def _normalize_traditional_text(text: str) -> str:
        ...
        replacements = {
            "导": "導", "变": "變", "达": "達", "发": "發", "会": "會", "国": "國",
            "围": "圍", "园": "園", "报": "報", "总": "總", "经": "經", "济": "濟",
            "门": "門", "为": "為", "与": "與", "个": "個", "气": "氣", "树": "樹",
            "体": "體", "现": "現", "应": "應", "议": "議", "环": "環", "境": "境",
            "统": "統", "计": "計", "划": "畫", "刘": "劉", "陈": "陳", "张": "張",
            "广": "廣", "东": "東", "义": "義", "业": "業", "众": "眾", "头": "頭",
            "厂": "廠", "万": "萬", "这": "這", "术": "術", "数": "數", "来": "來",
            "无": "無", "对": "對", "开": "開", "关": "關", "项": "項", "护": "護",
            "产": "產", "资": "資", "讯": "訊", "许": "許", "处": "處", "称": "稱",
            "见": "見",
        }
        out = text or ""
