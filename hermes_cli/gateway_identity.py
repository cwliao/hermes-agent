...
_UNKNOWN_MARKER_RE = re.compile(r"^(?:RELEASE_[A-Z0-9]+|\.hermes-release-[A-Za-z0-9]+)$")
...
def _marker_values(release_path: Path) -> tuple[str, str | None]:
    ...
    try:
        ...
        for child in release_path.iterdir():
            if child.is_file() and _UNKNOWN_MARKER_RE.match(child.name) and child.name not in KNOWN_MARKERS:
                raise GatewayIdentityError(f"unrecognized release marker: {child.name}")
    ...
    if not values:
        raise GatewayIdentityError(f"release marker missing: {release_path}")
...
def identity_from_project(project_root: Path) -> GatewayIdentity:
    ...
    try:
        ...
        if not str(exc).startswith("release marker missing:"):
            raise
...
def active_gateway_identity(
    ...
    *,
    ...
    if working_dir in legacy_roots:
        identity = identity_from_project(working_dir)
    else:
        try:
