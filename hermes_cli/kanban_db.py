...
def create_task(
    ...
    ``completion_contract``: optional acceptance-contract text recorded at
    creation time; unrelated to notification-delivery origin above.
    """
...
def _archive_task_in_txn(
    ...
    *,
    ...
    Returns ``(archived, run_id)``; ``run_id`` is the run closed by this call
    (or None), for a caller that also needs to log worker termination against
    the same run.

    ``allowed_statuses`` accepts either an iterable of statuses or a bare
...
def archive_task(
    ...
    *,
    ...
    """Archive a task; on the top-level call (not ``_within_transaction``), a
    *running* task's host-local worker is terminated.
    ...
    tracked anymore (#76196). Snapshot pid+claim before the archive txn so the
    ...
    A caller passing ``_within_transaction`` already owns the surrounding txn
    (e.g. the swarm worker-deadline excuser) and must handle any worker
    termination itself; this only performs the in-txn status flip for it.
    ...
    if _within_transaction:
        archived, _run_id = _archive_task_in_txn(
    ...
    with write_txn(conn):
        ...
        archived, run_id = _archive_task_in_txn(
            conn, task_id, allowed_statuses=_allowed_statuses,
        )
        if not archived:
            return False
