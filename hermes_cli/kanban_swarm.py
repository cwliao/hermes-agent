...
# beats two or four") -- the previous 600s external-lane default was 2x the
# previous 300s native default, not a value derived from a successful
# external-lane run's actual step count (no such run was observed in that
# investigation). The two defaults are now intentionally aligned at 1200s so
# a dispatcher-level timeout does not preempt a legitimately slow-but-successful
# vLLM request.
#
# DEFAULT_WORKER_MAX_RUNTIME_SECONDS itself was previously raised from 120 to 300 on
...
# external-CLI subprocess overhead, but 120s was too tight for anything beyond
...
_INTERNAL_LENGTH_MARKER_RE = re.compile(
    r"(?:（\s*\d+\s*字\s*）|\(\s*\d+\s*字\s*\))"
)
_QUOTE_PAIRS = (("「", "」"), ("“", "”"), ("（", "）"), ("(", ")"))
...
def validate_swarm_output_text(
    ...
    *,
    ...
    """Return a narrow, contract-scoped formatting rejection reason.

    This is deliberately not a Telegram/global sanitizer. It runs only for
    lane worker and synthesizer contracts, and it preserves the original text
    for audit. Ordinary parentheses/quotes remain valid when balanced.
    """
...
def swarm_output_metadata(
    ...
    *,
    ...
    """Return additive, non-user-visible validation metadata for a lane result."""
    if not contract or contract.get("role") not in {"worker", "synthesizer"}:
        return None
    value = (text or "").strip()
    ...
    return {
        ...
        "format_valid": reason is None,
        "validation_reason": reason or "ok",
    }
...
def validate_completion(
    ...
    *,
    ...
    # Worker completions historically allow metadata-only handoffs; validate
    # their text when a summary/result is actually supplied. Synthesizers are
    # checked for non-empty result in the role branch below.
...
def _create_swarm_uncommitted(
    ...
    *every* parent is ``done`` or ``archived``, and a ``blocked`` worker
    ...
    # Check for active swarms in this session right before creating the root task.
    # Runs under write_txn (BEGIN IMMEDIATE) so this check-and-create is atomic
    # across concurrent processes/threads sharing the same DB and session.
    ...
    for spec, expected_skill in zip(worker_specs, resolved_skills):
        ...
        if spec.toolsets:
            requested_toolsets = [
                str(name).strip() for name in spec.toolsets if str(name).strip()
            ]
            if requested_toolsets:
                worker_body += (
                    "\n"
                    + WORKER_TOOLSETS_PREFIX
                    + json.dumps(requested_toolsets, ensure_ascii=False)
                )
