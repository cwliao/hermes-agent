...
async def _handle_ingest(raw_args: str) -> str:
    ...
    try:
        ...
        result = ingest_document_to_docubot(
            ...
            # T0123: message_id must identify the Telegram event, not only the
            # pasted content. message_id goes first because
            # build_idempotency_key() truncates normalized stable_key to 32
            # chars; putting it first ensures it survives that truncation.
            stable_key=f"{message_id}-inline-text",
            multipart=True,
        )
