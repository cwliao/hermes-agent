...
def review(args: argparse.Namespace) -> tuple[int, dict[str, Any]]:
    ...
    try:
        ...
        return 1, {"phase": "review", "status": metadata["status"], "run_id": run_id, "candidate_path": str(candidate_path), "review_ref": review_ref, "candidate": metadata, "next_step": "依 error_code 修正後重新執行 review-only；不要 apply。"}
    finally:
        if worktree is not None:
            _git_try(repo, ["worktree", "remove", "--force", str(worktree)])
            shutil.rmtree(worktree, ignore_errors=True)

    return 0, {"phase": "review", "status": "PENDING", "run_id": run_id, "candidate_path": str(candidate_path), "review_ref": review_ref, "candidate_sha": metadata["candidate_sha"], "release_id": metadata["release_id"], "next_step": f"檢查 candidate diff，人工核准後以 run_id={run_id} 執行 apply。"}
