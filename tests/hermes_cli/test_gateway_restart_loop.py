...
class TestRestartLoopGuard:
    ...
    def test_spread_boots_never_trip(self):
        """Boots spread wider than the chain gap never trip. Gaps must
        exceed max_gap_seconds (default 300s, added by #81642's
        gap-chained pruning), not just window_seconds -- a 70s gap
        (this test's original spacing, pre-#81642) is well inside the
        default 300s gap and would now deliberately chain, since
        catching exactly that kind of slower-than-window cycle is the
        point of that fix."""
        import gateway.restart_loop_guard as rlg
        assert rlg.check_and_record(3, 60, now=1000.0) is False
        assert rlg.check_and_record(3, 60, now=1400.0) is False
        assert rlg.check_and_record(3, 60, now=1800.0) is False
