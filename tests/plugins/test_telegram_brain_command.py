...
@pytest.mark.asyncio
async def test_brain_source_command_reaches_plugin_without_rewriting_token(monkeypatch):
    adapter = _make_adapter(monkeypatch)
    called = {}
    ...
    async def fake_brain(query, **kwargs):
        called["query"] = query
        called["kwargs"] = kwargs
        return {"status": "error", "code": "invalid_request", "message": "invalid"}

    monkeypatch.setattr("plugins.platforms.telegram.adapter._handle_brain", fake_brain)
    await adapter._handle_command(
        SimpleNamespace(update_id=17, message=_make_message("/brain source signed.ref details")),
        None,
    )

    assert called["query"] == "source signed.ref details"
    assert called["kwargs"] == {"user_id": "101", "chat_id": "101", "chat_type": "dm"}
    adapter.send.assert_awaited_once_with("101", "invalid", reply_to="7")
