...
def _reject_downstream_swarm_mutation(tool_name: str) -> Optional[str]:
    ...
    if role in {"verifier", "synthesizer"}:
        return tool_error(
            ...
            "assigned task. Do not create child tasks, links, reviews, or artifacts."
        )
    return None
...
def _reject_in_flight_swarm_topology_mutation(
    ...
    if session_key:
        ...
        roots = conn.execute(
            "SELECT id FROM tasks WHERE origin_platform = ? AND origin_chat_id = ? "
            "AND status != 'archived'",
            (platform, chat_id),
        ).fetchall()
    ...
    for row in roots:
        ...
        internal_nodes = {
            str(task_id).strip()
            for task_id in topology.get("worker_ids", [])
        ...
        if matched:
            ...
            return tool_error(
                ...
                "Do not create substitute or workaround tasks for in-flight swarm work; "
...
KANBAN_SWARM_SCHEMA = {
    ...
    "description": (
        ...
        "synthesizer. Lane worker skills are filled automatically. Orchestrator-only."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "goal": {"type": "string", "description": "The swarm's overall goal."},
            "workers": {
                "type": "array",
...
_ORCHESTRATOR_TOOLS = frozenset({"kanban_list", "kanban_unblock", "kanban_swarm"})
_TOOLS = (
    ...
    ("kanban_swarm", KANBAN_SWARM_SCHEMA, _handle_swarm, "🐝"),
